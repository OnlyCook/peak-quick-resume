    ...
    internal static class MessagesLocalization
    {
        ...
        private static readonly Dictionary<MsgKey, string[]> _table = new Dictionary<MsgKey, string[]>
        {
            ...
            [MsgKey.CheckpointModStillInstalled] = new[]
            {
                ...
                "PEAK Checkpoint Save 仍已安裝。Quick Resume 不再需要它，可以完全獨立運作；兩個模組會各自獨立儲存與記錄日誌，因此你會看到重複的日誌訊息和存檔。這沒有危害（不會產生邏輯衝突），但解除安裝 PEAK Checkpoint Save 即可消除這些重複。",
            ...
            [MsgKey.CheckpointModStillInstalledShort] = new[]
            {
                ...
                "PEAK Checkpoint Save 仍已安裝，但已不再需要。你會看到重複的存檔和日誌，這沒有危害。按 {0} 查看詳情。",
            ...
            [MsgKey.GameUpdatedSavesMayBeWrong] = new[]
            {
                ...
                "遊戲已更新至 {0}：你現有的存檔現在可能會載入錯誤的島嶼，因為地圖池很可能已隨更新輪換。",
            ...
            [MsgKey.GameUpdateDialogSuppressed] = new[]
            {
                ...
                "PEAK 有可用更新。在所有人使用完這個存檔之前，請不要關閉或更新遊戲。現在更新可能會導致此存檔載入錯誤的島嶼，甚至完全損壞。",
