...
{
    /// <summary>
    /// Crossfades the game's own "LOADING..." screen prefab in and out around a Quick
    /// Resume teleport, instead of an abrupt instant on/off overlay. Bypasses
    /// <c>LoadingScreenHandler.Load</c>/<c>LoadingScreen.LoadingRoutine</c> (they drive
    /// Photon scene-load machinery we don't want); instead pulls the prefab directly and
    /// drives its <c>CanvasGroup.alpha</c> and dot-cycling text ourselves, showing
    /// "LOADING SAVE" (<see cref="MsgKey.LoadingSaveScreen"/>) instead of the hardcoded default.
    /// </summary>
            ...
            else
            {
                try
                {
                    ...
                    else
                    {
                        ...
                        // The prefab's own Animator otherwise keeps pinning CanvasGroup.alpha at 1,
                        // fighting our script-driven lerp below (screen "just disappears" instead
                        // of crossfading). Disabling it hands full control of the CanvasGroup to us.
