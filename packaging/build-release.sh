...
# dist/nexus/PEAKQuickResume-<version>-nexus.zip, which is the same file set
# as the Thunderstore zip but nested one level under an
# OnlyCook-PEAKQuickResume/ folder, so extracting it straight into
...
DIST="$REPO_ROOT/dist"
NEXUS_FOLDER="OnlyCook-PEAKQuickResume"
...
NEXUS_OUT="$NEXUS_DIST/PEAKQuickResume-$VERSION-nexus.zip"
rm -f "$NEXUS_OUT"
